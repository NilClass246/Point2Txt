...
class PointTransformer(nn.Module):
    ...
    def load_checkpoint(self, bert_ckpt_path):
        ...
        # Handle DDP prefix/model prefix
        base_ckpt = ckpt['state_dict'] if 'state_dict' in ckpt else ckpt
        
        for k, v in base_ckpt.items():
            if k.startswith('module.point_encoder.'):
                state_dict[k.replace('module.point_encoder.', '')] = v
            elif k.startswith('point_encoder.'):
                state_dict[k.replace('point_encoder.', '')] = v
            else:
                state_dict[k] = v

        # Standard torch strict loading
        missing_keys, unexpected_keys = self.load_state_dict(state_dict, strict=False)
        
        if len(missing_keys) > 0:
            print(f"Warning: Missing keys in PointBERT: {missing_keys}")
        print(f"PointBERT weights loaded from {bert_ckpt_path}")
